...
import com.qep.measureprocessingservice.util.service.FhirCodeMapperServiceStu3;
    ...
    private void performRetrieve(Iterable result, JsonObject results) {
        ...
        while (it.hasNext()) {
            ...
            findings.add(fhirContext
                    .newJsonParser()
                    .setPrettyPrint(true)
                    .encodeResourceToString((IBaseResource)it.next()));
        }
...
//        options.add(CqlTranslator.Options.EnableDetailedErrors);
    ...
    public String evaluateCql(String requestData) throws JAXBException, IOException, ParseException {
        ...
        JsonArray jsonArray = json.get("parameter").getAsJsonArray();   //.has("valueString") ? json.get("valueString").getAsString() : null;
        String code = jsonArray.get(0).getAsJsonObject().get("valueString").getAsString();

        String terminologyServiceUri = jsonArray.get(4).getAsJsonObject().get("valueString").getAsString();
        ...
        String patientId = jsonArray.get(1).getAsJsonObject().get("valueString").getAsString();
